...
//  freeDebugPins();
// safe remap to SWD-only (masked)
RCC->APB2ENR |= RCC_APB2ENR_SYSCFGEN;
(void)RCC->APB2ENR;
uint32_t tmp = SYSCFG->CFGR1;
tmp = (tmp & ~(0b111 << 8)) | (0b001 << 8);
SYSCFG->CFGR1 = tmp;
...
GPIOB->MODER &= ~(3U << (4*2));
GPIOB->MODER |=  (1U << (4*2));   // output
GPIOB->OTYPER &= ~(1U << 4);      // push-pull
GPIOB->PUPDR &= ~(3U << (4*2));   // no pull
