...
const T = {p: prim,  s: sec, c: core,  
           f:     50  ,   // frequency
           pwr:   900 ,  // Rated power (W)
           eff:   0.87,  // Efficiency
           tpv:   1,    // Turns per volt
}

console.log("TStart", T) ;
...
var Acore = 1.152e2 * (T.pwr)^0.5 ;  // mm2

//Calculate Number of Turns per Volt paameter
T.tpv = 1.0 / (4.44e-6 * T.f * Acore * T.c.B ) ;
console.log(T.tpv , T.p.V, T.tpv * T.p.V);
...
var Awindow = Acopper * 1.03 ;  // Allows for insulation etc.
